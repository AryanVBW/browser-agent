...
try:
    from brouser_agent.config.paths import path_manager
    from brouser_agent.config.settings import settings_manager
    from brouser_agent.config.constants import APP_NAME, APP_VERSION
    from brouser_agent.utils.logging_config import setup_logging, get_logger, shutdown_logging
...
def create_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
  brouser-agent --gui                    # Launch GUI
  brouser-agent --url https://example.com --task "Get page title"
  brouser-agent --config                 # Show configuration
  brouser-agent --setup                  # Run setup wizard
  brouser-agent --version                # Show version
        """
    )
...
def run_setup():
    ...
    try:
        ...
        from brouser_agent.core.setup import SetupWizard
...
def execute_task(url: str, task: str, args) -> int:
    ...
    try:
        ...
        from brouser_agent.core.agent import BrowserAgent
